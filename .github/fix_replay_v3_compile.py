...
"""
    ...
    deploy = Path("src/deploy.rs")
    deploy_source = deploy.read_text()
    if "use std::io::Write;" not in deploy_source:
        marker = "use clap::Args;\\n"
        if marker not in deploy_source:
            raise RuntimeError("deploy import marker not found")
        deploy_source = deploy_source.replace(marker, marker + "use std::io::Write;\\n", 1)
    deploy.write_text(deploy_source)

    status_bar = Path("src/ui/panels/status_bar.rs")
    status_source = status_bar.read_text().replace("    let mut advance =", "    let advance =", 1)
    status_bar.write_text(status_source)
